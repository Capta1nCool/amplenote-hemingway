      ...
      run: async function (app, noteUUID) {
        ...
        paragraphs.forEach(p => {
          ...
          p.innerHTML = p.innerHTML.replace(complexPattern, (match) => {
            ...
            rfnotes += `\n\n
[^${rfnoteCount}]: [${match}]()
  Use simpler alternatives or remove: ${JSON.stringify(alternatives)}\n
`;

            return `<mark style="background-color:#211831;color:#8D68DA;">[${match}][^${rfnoteCount}]</mark>`;
          });
        });
        ...
        console.log(tempDiv.innerHTML, markdown);
        ...
        const uuid = await app.createNote(`Hemingway - ${note.name}`, ["hemingway/analysis"]);
